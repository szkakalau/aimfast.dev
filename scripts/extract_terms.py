...
def _load_raw_signals(date_str: str) -> list[dict]:
    ...
    if not raw_path.exists():
        print(f"[TermExtract] {raw_path} 不存在")
        return []
...
def _call_llm_extract(signals: list[dict]) -> list[dict]:
    ...
    user_prompt = _build_llm_prompt(signals)
    ...
    try:
        ...
        print(f"[TermExtract] LLM 提取 {len(terms)} 个命名实体")
        return terms

    except json.JSONDecodeError as e:
        print(f"[TermExtract] LLM 返回 JSON 解析失败: {e}")
        print(f"  原始响应前 200 字符: {raw[:200] if 'raw' in dir() else 'N/A'}")
        return []
    except Exception as e:
        print(f"[TermExtract] LLM 调用失败: {e}")
        return []
