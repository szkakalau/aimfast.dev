...
### GEO 铁律（AI 引擎可引用性 — 最重要）
- **每句必须有句末标点**: 句号、问号或感叹号。禁止连续逗号分句而不用句号收尾。整篇文章不能出现未闭合的句子。
- **每段至少一句短句**: 不超过 15 字。AI crawl 用短句做 extractive summary。
- **可引用句密度**: 每 200 字至少 1 句可直接引用的事实陈述（数字、日期、对比、名字）。
- **被动语态抑制**: 每段最多 1 处被动语态。优先主动句。
- **禁止 run-on 句**: 单句不超过 40 字。超过就拆成两句。
- **GEO 友好 H2**: 每个 H2 标题本身就是一个完整的问题或陈述（如「为什么 4000 个开发者用评论找工作」「这个痛点值多少钱」），能被 LLM 直接提取为 FAQ 候选。
