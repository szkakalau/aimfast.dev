...
def generate_sitemap() -> int:
    ...
    # Skip reports that are auto-generated placeholders (LLM unavailable at pipeline time)
    PLACEHOLDER_MARKER = "自动生成于 AimFast.Dev"
    if CONTENT_REPORTS.exists():
        for f in sorted(CONTENT_REPORTS.glob('*-en.md')):
            ...
            # Skip thin / auto-generated placeholder reports (< 1KB or placeholder marker)
            try:
                raw = f.read_text(encoding='utf-8')
                if len(raw) < 800 or PLACEHOLDER_MARKER in raw:
                    print(f"  [SEO] Skipping thin/placeholder report: {date}")
                    continue
            except Exception:
                pass
            ...
            if has_zh:
                # Also check ZH version for placeholder content
                try:
                    zh_raw = zh_file.read_text(encoding='utf-8')
                    if len(zh_raw) < 800 or PLACEHOLDER_MARKER in zh_raw:
                        continue
                except Exception:
                    pass
                urls.append({
    ...
    # Collect trends from both content/trends/*.md AND trend_terms.json.
    # The JSON is the source of truth — md files are generated by the pipeline
    # and may lag behind (37 terms have data but no md file).
    ...
    if CONTENT_TRENDS.exists():
        ...
        # First pass: trends with md files (verified content)
        for f in sorted(CONTENT_TRENDS.glob("*.md")):
            if "-en" in f.name:
                continue
            slug = f.stem
            if slug not in terms_by_slug:
                # Skip trends without data — these render "Trend not found"
                print(f"  [SEO] Skipping trend without data: {slug}")
                continue
            seen_trends.add(slug)
            lastmod = terms_by_slug[slug]
        ...
        # Second pass: trends with data but no md file yet (pending pipeline)
        for slug, lastmod in sorted(terms_by_slug.items()):
            if slug in seen_trends:
                continue
            urls.append({
                'loc': f'{BASE_URL}/trends/{slug}/',
                'lastmod': lastmod,
